...
/**
 *This class sets up and runs the JunglePark simulation by extending the PApplet class.
 *It provides the GUI and necessary method calls to handle all the actions within the simulation.
 * 
 * @author Mouna Kacem
 *
 */
public class JunglePark extends PApplet {
   ...
   */
  public void clear() {
    for (int i = 0; i < listGUI.size(); i++) { //runs through the list of objects
      if (listGUI.get(i) instanceof Animal) { //if the object is an animal it will remove it
        listGUI.remove(i);
        i--;
      }
    }
