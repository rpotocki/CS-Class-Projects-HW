...
public class Animal implements ParkGUI {
  ...
  /**
   * This method checks if an animal is within a range of another animal
   * 
   * @returns true if animal is in range and false if not
   */
  public boolean isClose(Animal otherAnimal, int range) { 
    float animalY = this.getPositionY();
    float animalX = this.getPositionX();
    float otherAnimalX = otherAnimal.getPositionX();
    float otherAnimalY = otherAnimal.getPositionY();
    if (((otherAnimalX <= animalX + range) && (otherAnimalX >= animalX - range)) && ((otherAnimalY <= animalY + range) && (otherAnimalY >= animalY - range))) {
      return true;
    }
    return false;
